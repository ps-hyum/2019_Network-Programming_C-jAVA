...
void HandleClient(int clntSocket, int clntID);
void* thread_func(void* clientSocket);

int main(int argc, char* argv[])
{
	head_room = rear_room = NULL; head_wait = rear_wait = NULL; head_ftp = rear_ftp = NULL;
	clntID = 0;
...
void* thread_func(void* clientSocket) {
	...
	printf("client[%d]가 로그인 하였습니다.\n", clntID);
	HandleClient(clntSock, clntID++);
	close(clntSock);
}